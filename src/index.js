module.exports = function(schema, option) {
  ...
  const parseCondition = (condition, render) => {
    ...
    render = render.replace(/^<\w+\s/, `${render.match(/^<\w+\s/)[0]} *ngIf="${_condition}" `);
    return render;
  };
  ...
  const parseLoop = (loop, loopArg, render) => {
    ...
    render = `
      ...
      *ngFor="(${loopArgItem}, ${loopArgIndex}) in ${data}"  
      ${keyProp}
      ${render.slice(tagEnd)}`;
  ...
  return {
    panelDisplay: [
      {
        panelName: `index.component.html`,
      ...
      {
        panelName: 'index.component.less',
